 ...
 *   2. <script src="novus-core.js"></script>
 *   3. <script> /* page code * / </script>
 */
  ...
  function _buildModal() {
    ...
    overlay.innerHTML = `
      <div id="_nv_box" style="
        background:var(--surface,#1a1f2e);width:100%;max-width:400px;
        border-radius:20px;border:1px solid var(--border,rgba(255,255,255,.055));
        box-shadow:0 32px 80px rgba(0,0,0,.75);overflow:hidden;
        ...
        <style>
          ...
          ._nv_label{font-size:13px;font-weight:600;color:var(--text,#e1e5f0);font-family:'DM Sans',sans-serif}
          ._nv_field{width:100%;padding:12px 14px;
            background:var(--surface-2,#1f2538);border:1.5px solid var(--border,rgba(255,255,255,.055));
            border-radius:12px;font-size:15px;font-weight:600;color:var(--text,#e1e5f0);
            outline:none;font-family:'DM Sans',sans-serif;transition:border-color .15s;box-sizing:border-box}
        </style>
        <div style="padding:16px 20px;border-bottom:1px solid var(--border,rgba(255,255,255,.055));
                    display:flex;justify-content:space-between;align-items:center">
          <span style="font-size:15px;font-weight:800;color:var(--text,#e1e5f0);font-family:'DM Sans',sans-serif">⚙ Settings</span>
          <button onclick="NovusSettings.closeModal()" style="background:none;color:var(--text-3,#4a5568);font-size:18px;font-weight:900;border:none;cursor:pointer;line-height:1">✕</button>
        </div>
        <div style="padding:20px;display:flex;flex-direction:column;gap:16px">
          <div>
            <div style="font-size:8px;font-weight:800;text-transform:uppercase;letter-spacing:1.5px;color:var(--text-3,#4a5568);margin-bottom:7px;font-family:'DM Sans',sans-serif">Your Name / Badge ID</div>
            <input id="_nv_name" type="text" class="_nv_field" placeholder="Enter your name…"
              onfocus="this.style.borderColor='var(--accent,#00e5ff)'"
              onblur="this.style.borderColor='var(--border,rgba(255,255,255,.055))'">
          </div>
          <div class="_nv_row">
            <span class="_nv_label">Haptic Feedback</span>
            <button id="_nv_haptic" class="_nv_toggle" onclick="NovusSettings._toggle('hapticFeedback','_nv_haptic')"><div class="_nv_knob"></div></button>
          </div>
          <div class="_nv_row">
            <span class="_nv_label">Compact Mode</span>
            <button id="_nv_compact" class="_nv_toggle" onclick="NovusSettings._toggle('compactMode','_nv_compact')"><div class="_nv_knob"></div></button>
          </div>
          <div style="font-size:9px;color:var(--text-3,#4a5568);text-align:center;font-family:'DM Sans',sans-serif;margin-top:4px">
            Novus Foods · Plant 1730 · Supabase v2
          </div>
        </div>
        <div style="padding:14px 20px;border-top:1px solid var(--border,rgba(255,255,255,.055))">
          <button onclick="NovusSettings.saveAndClose()" style="
            ...
            font-family:'DM Sans',sans-serif;text-transform:uppercase;letter-spacing:.5px;
            transition:filter .15s">Save Settings</button>
        </div>
      </div>`;
